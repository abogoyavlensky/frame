...
> **Status: COMPLETED** (2026-07-12) — see summary at the end.
...
- [x] **Step 3: Copy `R/root/` into `template/`**
...
- [x] **Step 4: Confirm the verbatim files are tag-free**
...
- [x] **Step 1: Create the templated `src` dir**
  `mkdir -p` the `src/{{project-name|snake_case}}` and `auth` subdirs (literal names with the compact tags).

- [x] **Step 2: Copy base + auth files and rewrite `{{main/ns}}`→`{{ project-name }}`**
...
- [x] **Step 3: Inline `{{sql-result-set-config}}` in `db.clj`**
...
- [x] **Step 4: Verify no stray deps-new tags remain**
...
- [x] **Step 1: Copy base tests, rewrite `{{main/ns}}`→`{{ project-name }}`**

- [x] **Step 2: Inline `{{test-utils-db-setup}}` in `test_utils.clj`**
...
- [x] **Step 3: Copy the 6 auth tests under conditional names, rewrite ns**
  Each literal filename is `{%if auth%}<name>_test.clj{%endif%}`. All use `{{main/ns}}` only.

- [x] **Step 4: Verify tags**
...
- [x] **Step 1: `deps.edn` — three inline substitutions**
...
- [x] **Step 2: `bb.edn` — three inline substitutions**
...
- [x] **Step 2: Migration files (conditional names, verbatim content)**
...
- [x] **Step 3: `db/.gitkeep` and `docker-compose.yaml`**
...
- [x] **Step 1: `.kamal/deploy.yml` + `secrets`**
...
- [x] **Step 2: `deploy.yaml` with `{% raw %}` blocks**
...
- [x] **Step 3: `README.md`, `Dockerfile`, `LICENSE`, `manifest.json`**
...
- [x] **Step 4: Verify no unmapped tags remain anywhere**
...
- [x] **Step 2: DaisyUI JS conditional names**
...
- [x] **Step 2: Clean untracked deps-new cache**
...
- [x] **Step 3: Write a verification script**
...
- [x] **Step 4: Run and fix until clean**
...
- [x] **Step 5: Confirm deps-new fallback still passes**
  Run in `$CSL`: `bb test`
  Expected: PASS (template.edn still valid; deps-new files untouched).

- [x] **Step 6: Commit any template fixes**
...
**Implemented and verified.** deps-new and frame now generate **byte-identical** projects from `clojure-stack-lite` across the **full 16-combo matrix** (db × auth × daisyui × deploy, both hyphenated and plain names); `diff -r` is empty for every combo. The deps-new files are untouched and `bb test` still passes (1 test, 0 failures), so the fallback is intact.

**clojure-stack-lite (`migrate-to-frame`):** `frame.edn` (vars `db`/`auth`/`daisyui`/`deploy`/`developer`, `:raw` globs for JS/images/GitHub-Actions) + `template/` mirroring the deps-new output, with all 13 deps-new substitutions expressed as inline `{% case %}`/`{% if %}` blocks, conditional path segments for optional files/dirs, and `{% raw %}` blocks for the mixed `deploy.yaml`. README gained a frame-usage subsection.

**frame (`master`):** one bug fix — `binary?` (`src/frame/generate.lg`) now treats **empty** content as text instead of crashing (`(and (seq content) …)`); empty files (`.gitkeep`) previously threw `"zero? requires a number, got "`. Regression test added; full suite green (153 tests).

**Deviations (recorded under their tasks / here):**
- **frame source change was required** (the plan predicted "no frame changes"). The `binary?` empty-file crash is a genuine engine bug that any template with an empty file hits; the demo fixture had none, so it was latent. Distorting the template to avoid empty files would break parity, so the fix belongs in frame. Minimal, tested, intent-preserving.
- **README trailing-newline** needed an empty `{% else %}` before `{% endif %}`. The `readme-deploy-kamal` chunk ends in `\n`, which left `{% endif %}` alone on its line — frame's line-ownership rule then ate the file's final newline (deps-new keeps it). Making `{% endif %}`'s neighbor a tag (via empty `{% else %}`) suppresses ownership. This is the one spot where deps-new's pure string-replace and frame's whitespace rule diverge at EOF.
- `input.css` uses two conditional-named files (default vs daisyui) rather than one file with an inline `{% if %}` — deps-new does a whole-file swap (RENAMINGS), and two files are byte-safe with zero whitespace-rule risk.
- `mise trust` was needed on the CSL repo before deps-new (`clojure`) would run under the verification harness — environment setup, not a template change.

**What the plan could have specified better:** it asserted "no changes to frame itself," but should have flagged empty-file handling as a risk — the template ships several `.gitkeep` files, and `binary?`'s empty-content path was untested. Checking frame's `binary?`/`seq`-on-empty behavior during planning (as the prior migration checked `developer` derivation) would have predicted the one engine fix before verification surfaced it. Everything else in the plan held up exactly.
